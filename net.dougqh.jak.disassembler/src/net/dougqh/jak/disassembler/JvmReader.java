...
public final class JvmReader {
	...
	public final Iterable< JavaType > list() {
		try {
			return new TransformIterable< InputStream, JavaType >( this.locator().list() ) {
				@Override
				protected final JavaType transform( final InputStream in ) {
					try {
						try {
							return JavaType.create( in );
						} finally {
							in.close();
						}
					} catch ( IOException e ) {
	...
	public final JavaType read( final String name ) throws IOException {
